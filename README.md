...
| `nishi` | 西店スタッフ | 発注 / 注文 / 売上 / カレンダー / タイムカード / 日報 / 給与明細 / 給料 |
| `minami` | 南店スタッフ | 発注 / 注文 / 売上 / カレンダー / タイムカード / 日報 / 給与明細 / 給料 |
| `hq1` | 本部・野菜担当 | hq / カレンダー / タイムカード / 給与明細 / 給料 |
| `hq2` | 本部・果物担当 | hq / カレンダー / タイムカード / 給与明細 / 給料 |
| `hq3` | 本部・餅乾物菓子担当 | hq / カレンダー / タイムカード / 給与明細 / 給料 |
| `all` | 全権限（オーナー） | 全コマンド |
...
| 「給料」 | freee の給与明細（Web明細）を直接開く URL。`https://p.secure.freee.co.jp/payroll_statements#/<事業所ID>/<年>/<月>/employees/<User.freeeId>` を組み立てて返す。対象月は JST の当月。`freeeId` 未登録なら管理者に問い合わせる旨を返信。閲覧には freee へのログインが必要。 |
