...
| `nishi` | 西店スタッフ | 発注 / 注文 / 売上 / カレンダー / タイムカード / 日報 / 給与明細 |
| `minami` | 南店スタッフ | 発注 / 注文 / 売上 / カレンダー / タイムカード / 日報 / 給与明細 |
| `hq1` | 本部・野菜担当 | hq / カレンダー / タイムカード / 給与明細 |
| `hq2` | 本部・果物担当 | hq / カレンダー / タイムカード / 給与明細 |
| `hq3` | 本部・餅乾物菓子担当 | hq / カレンダー / タイムカード / 給与明細 |
| `all` | 全権限（オーナー） | 全コマンド |
...
| 「タイムカード」 | 日報システムの勤怠打刻 URL。`User.freeeId` で `nippo.staff_private` を引き、`clock_token` があれば個別 URL（`/store/<branch>/clock/<token>`）を返す。店舗スタッフ・本部は個別 URL 1 本に差し替え、`all` は個別 URL＋各店舗の共通 URL。個別 URL が作れない場合、店舗が特定できれば（店舗ロール／所属店舗設定済み）その店舗の共通 URL、特定できなければ西・南の共通 URL を両方返す。外部システムのため `lineUserId` は付けない。 |
