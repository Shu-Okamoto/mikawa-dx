...
export default function HqPage() {
  const { user, loading, authFetch, logout } = useAuth('hq')
  ...
  const fetchOrders = useCallback(async () => {
    ...
    const res  = await authFetch(
      `/api/daily-orders/hq?category=${encodeURIComponent(user.category || '')}`
    )
  ...
  const copyForLine = () => {
    ...
    let text = `【${user?.category}発注】${dateStr}\n`
    items.forEach((item) => {
      const qty = adjusted[item.productId] || item.totalQty
      if (qty > 0) {
        text += `${item.productName}: ${qty}${item.unit}\n`
      }
    })
