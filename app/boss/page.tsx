...
function BossPageContent() {
  ...
  const { totalAmount, totalSouzai, totalMochi, totalHana, totalCust, charts } =
    useMemo(() => {
      const stores = ['西店', '南店']
      let totalAmount = 0
      let totalSouzai = 0
      ...
      const buildChart = (
        d: { amount:number; souzai:number; mochi:number; hana:number; customerCount:number },
      ) => {
      ...
      charts['合計'] = buildChart({
        ...
        hana: totalHana, customerCount: totalCust,
      })

      return { totalAmount, totalSouzai, totalMochi, totalHana, totalCust, charts }
    }, [data])
